        ...
        for i, feat_item in enumerate(self.feature_list["features"]):
            ...
            try:
                if feat_type == "newSketch":
                    ...
                    raise NotImplementedError(
                        self.data_id, "unsupported feature type: {}".format(feat_type)
                    )
            except Exception as e:
            ...
            result["sequence"].append(
                {"index": i, "type": feat_dict["type"], "entity": feat_Id}
            )
        return result
...
class SketchParser(object):
    ...
    def __init__(self, client, feat_data, did, wid, eid, data_id=None):
        ...
        self.plane = self.c.parse_face_msg(
            response.json()["result"]["message"]["value"]
        )[0]

        self.geo_topo = self.c.eval_sketch_topology_by_adjacency(
            did, wid, eid, self.feat_id
        )
    ...
    def _parse_edges_to_loops(self, all_edge_ids):
        ...
        while len(unvisited_edges) > 0:
            ...
            if (
                len(self.edge_table[cur_edge]["vertices"]) == 0
            ):  # no corresponding vertices
                pass
            else:
                loop_start_point, cur_end_point = (
                    self.edge_table[cur_edge]["vertices"][0],
    ...
    def _parse_edge_to_fusion360_format(self, edge_id):
        ...
        if edge_type == "Line":
            ...
            start_vec = np.array(
                self.vert_table[start_id]["param"]["Vector"]
            ) - np.array(edge_data["param"]["coordSystem"]["origin"])
            end_vec = np.array(self.vert_table[end_id]["param"]["Vector"]) - np.array(
                edge_data["param"]["coordSystem"]["origin"]
            )
    ...
    def parse_to_fusion360_format(self):
        ...
        transform_dict = {
            ...
            "y_axis": xyz_list2dict(
                list(np.cross(self.plane["normal"], self.plane["x"]))
            ),
        }
        ref_plane_dict = {}
        ...
        for item in self.geo_topo["faces"]:
            ...
            for loop in edge_ids_per_loop:
                curves = [
                    self._parse_edge_to_fusion360_format(edge_id) for edge_id in loop
                ]
        ...
        entity_dict = {
            ...
            "reference_plane": ref_plane_dict,
        }
        return entity_dict
