...
class MyClient(Client):
    ...
    def eval_entityID_created_by_feature(self, did, wid, eid, feat_id, entity_type):
        ...
        body = {
            ...
            + '           qCreatedBy(makeId("%s"), EntityType.%s)'
            % (feat_id, entity_type)
            + "       )"
            "   );"
            "}",
        ...
        entityIDs = [item["message"]["value"] for item in res_msg]
        return entityIDs

    def eval_bodydetails(self, did, wid, eid):
        """parse the B-rep representation as a dict"""
        res = self._api.request(
            "get", "/api/partstudios/d/{}/w/{}/e/{}/bodydetails".format(did, wid, eid)
        ).json()
        ...
        for body in res["bodies"]:
            ...
            face_params = self.parse_face_msg(
                face_entity.json()["result"]["message"]["value"]
            )
